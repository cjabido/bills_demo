...
export default function MarkReceivedModal({ source, onConfirm, onClose }: MarkReceivedModalProps) {
  const [amount, setAmount] = useState('');

  useEffect(() => {
    if (source) {
      setAmount(source.amount.toFixed(2));
    }
  }, [source]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-sm mx-4 bg-surface-1 border border-border-default rounded-2xl shadow-2xl shadow-black/10 animate-fade-in-up">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border-dim">
          <h2 className="text-base font-semibold text-text-primary">Mark as Received</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <p className="text-sm text-text-secondary">
            Confirm receipt of <span className="font-medium text-text-primary">{source.name}</span>
          </p>
          ...
          <div className="flex items-center gap-3 pt-1">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-2.5 rounded-lg bg-surface-2 hover:bg-surface-3 text-text-secondary text-sm font-medium border border-border-dim transition-all duration-150 cursor-pointer"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="flex-1 py-2.5 rounded-lg bg-accent-mint/15 hover:bg-accent-mint/25 text-accent-mint text-sm font-semibold border border-accent-mint/20 hover:border-accent-mint/30 transition-all duration-150 flex items-center justify-center gap-2 cursor-pointer"
            >
